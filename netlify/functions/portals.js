...
// API de gestão de portais (SIMPLIFICADA - apenas nome)
const { Pool } = require('pg');
const jwt = require('jsonwebtoken');

const pool = new Pool({
  connectionString: process.env.DATABASE_URL_FAMALICAO,
    ...
    if (event.httpMethod === 'POST') {
      ...
      if (!data.name || !data.name.trim()) {
        return {
          ...
          body: JSON.stringify({ 
            success: false, 
            error: 'Nome do portal é obrigatório' 
          })
        };
      }

      const query = `
        INSERT INTO portals (name, created_at, updated_at)
        VALUES ($1, $2, $3)
        RETURNING *
      `;
